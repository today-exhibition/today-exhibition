...
def thumbnail_to_s3(id, thumbnail_img):
    ...
    filename = image_download(id, thumbnail_img)
    bucket_name = BUCKET_NAME  
    original_file_key = filename.split('/')
    if len(original_file_key[-1]) > 4:
        image_type = original_file_key
    else:    
        image_type = original_file_key[1].split('.')[-1].lower()
    ContentType = get_img_type(image_type)
...
def image_download(id, image_url):
    local_directory = 'seoul_thumbnail_imgs/'
